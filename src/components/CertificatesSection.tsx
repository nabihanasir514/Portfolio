  ...
  const certificates = [
    {
      ...
      link: '/certificates/ICPC_Asia_Topi_Place_5.pdf', // Case-sensitive
      featured: true,
      badge: '5th Place',
    },
    {
      ...
      link: '/certificates/ICPC_Nabiha_Nasir.pdf', // Case-sensitive
      featured: true,
    },
    {
    ...
    },
    {
      ...
      link: '/certificates/Into_to_Software_Engineering.pdf', // Case-sensitive
    },
    {
      ...
      link: '/certificates/Foundations_of_UX_Design.pdf', // Case-sensitive
    },
    {
      ...
      link: '/certificates/Introduction_to_python.pdf', // Matches GitHub case
    },
  ];
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className={`text-center mb-12 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Certifications and credentials that validate my skills and commitment to continuous learning.
          </p>
        </div>
          ...
          {filteredCertificates.map((cert, index) => (
            <div
              ...
              className={`group glass-card rounded-2xl overflow-hidden transition-all ${
                cert.featured ? 'border-yellow-500/30 ring-1 ring-yellow-500/20' : 'hover:border-primary/30'
            ...
            >
              <div className={`h-24 bg-gradient-to-r ${cert.color} relative`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  {cert.featured ? <Trophy className="w-12 h-12 text-yellow-300" /> : <Award className="w-12 h-12 text-white/80" />}
                </div>
                <span className="absolute top-3 right-3 px-3 py-1 text-xs rounded-full bg-black/30 text-white">{cert.category}</span>
                {cert.badge && (
              ...
              <div className="p-6">
                <h3 className={`text-lg font-semibold mb-2 ${cert.featured ? 'text-yellow-400' : 'group-hover:text-primary'}`}>
                  {cert.title}
                </h3>
                <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                  <span className="font-medium">{cert.issuer}</span>
                  <span className="flex items-center gap-1"><Calendar className="w-4 h-4" />{cert.date}</span>
                </div>
                <a
                  href={cert.link}
                  target="_blank"
